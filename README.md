...
`server/config/server.svc.account` and just paste the config.yaml content in the consul store for that key in yaml and save.  
...
```typescript
  ...
  @ResolveField(() => Card)
  async create(@Args('input'); CreateCardInput, @Context(); GqlContext;): Promise<Card> {
    // @ts-ignore
