...
import { AuthServicesTypes, UserEntity, UserRepository } from '@server/repository';
import { generateVerificationCode } from '@server/common/utils/verification-code-generator';
import { CreateResponse, LoginServiceTypes } from '@server/proto-schema/account';
import { RegisterUserCommand } from '../../impl';
import { UserRegisteredEvent } from '@server/core/cqrs';
import { RpcException } from '@nestjs/microservices';
import { JwtService } from '@nestjs/jwt';
import { BillingsRpcClientService, RolesRpcClientService } from '@server/core';
...
export class RegisterUserHandler
    ...
    @Inject(RolesRpcClientService)
    private readonly roleClient: RolesRpcClientService;

    @Inject(BillingsRpcClientService)
    ...
    async execute(command: RegisterUserCommand): Promise<CreateResponse> {
        Logger.log(`Async ${command.constructor.name}...`, this.constructor.name);
        const { cmd } = command;
        ...
        try {
            ...
            console.log(error);
            Logger.log(error.message, this.constructor.name);
